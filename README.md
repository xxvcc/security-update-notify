...
- **Telegram 提醒中英双语同屏**：安装时可选择中文在前或英文在前，默认中文在前。
...
--notify-lang zh           # Telegram 中英双语提醒：中文在前（默认）
--notify-lang en           # Telegram 中英双语提醒：英文在前
...
安装后切换 Telegram 中英双语提醒显示顺序：

```bash
sudoedit /etc/security-update-notify/telegram.env
# 设置 NOTIFY_LANG=zh（中文在前）或 NOTIFY_LANG=en（英文在前）
```
