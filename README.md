...
- **中英文界面，单语显示**：安装、菜单、诊断在第一步选择语言（中文或英文，默认中文），之后整套终端交互只按所选语言显示，不再中英文混排；该选择同时作为 Telegram 提醒语言的默认值，可用 `--notify-lang` 单独覆盖。
...
安装器会先让你选择界面语言（中文或英文，默认中文）：它决定后续所有交互的显示语言，并作为 Telegram 提醒语言的默认值。随后会询问：
