...
尽力支持只表示 SUN 的代码路径兼容，管理员仍须确认订阅和安全源有效。Ubuntu 20.04 会自动核对本机 `esm-infra`；Amazon Linux 2023 仍需管理员跟踪并前移发行快照。未列出的 `ID_LIKE` 衍生版不会自动升级为正式支持，详细探测和回滚规则见[运维文档](docs/operations.md)。
