...
- Telegram：使用 `getMe` 验证 Bot Token，并用 `sendMessage` 验证 Chat ID 与权限；只读 `getMe` 会对连接重置、超时、HTTP 429 和 5xx 最多尝试三次。为避免重复消息，`sendMessage` 只在服务端明确返回 HTTP 429 时重试；传输错误、5xx 和响应中断作为临时失败返回，但不立即重发。持续的临时网络故障不会被误报为 Token 无效，也不会清空旧凭据；交互模式可重试、跳过本次预检或中止，非交互模式以退出码 `75` 失败并回滚；
