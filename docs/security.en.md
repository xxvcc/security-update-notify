...
The release `.sha256` file protects against accidental corruption or version mismatch. If your threat model includes a compromised download source, keep `sun.sh` signature verification enabled and do not use `--verify-signature off`. Go self-upgrade has no option to disable signature verification.
