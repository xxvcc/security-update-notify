...
- 为安装、配置和升级增加可持久恢复的事务日志：在停止现有 unit 或改写受管路径前保存完整回滚状态，普通错误、取消及首次终止信号会等待回滚完成；异常中断后，下一次安装只会恢复经完整校验且明确标记为安全的事务。若中断发生在包管理器状态尚未可信捕获的阶段，安装器会保留事务与私密凭据恢复材料，并让后续安装和卸载在产生 systemd 或文件副作用前失败关闭，要求管理员检查并完成人工恢复。
  Adds durable transaction recovery to installation, configuration, and upgrade. Complete rollback state is persisted before an existing unit is stopped or a managed path is changed, and ordinary errors, cancellation, or the first termination signal wait for rollback to finish. After an abrupt interruption, the next install restores only a fully validated transaction explicitly marked safe. If interruption occurs before package-manager state has been captured reliably, the installer retains the journal and private credential recovery material, and later install or uninstall attempts fail closed before systemd or filesystem side effects until an administrator inspects and completes recovery.
- 收紧特权运行和自升级的信任边界：root 生产进程忽略仅供隔离测试使用的 API、配置、凭据、状态、锁、日志及后端路径环境覆盖；安装器子进程只有通过重新绑定到规范锁 inode 并验证 flock 能力的继承描述符，才能复用父进程持有的运行锁。Go 自升级删除 SHA-256-only 逃生路径，始终要求可信系统路径中的 `gpg`、归档签名、内置公钥和固定指纹；引导安装与特权自升级均忽略调用者的 `TMPDIR`，只在逐级 no-follow 验证的 root 所有系统临时目录中创建私有工作区。
  Tightens privileged runtime and self-upgrade trust boundaries. Root production processes ignore API, configuration, credential, state, lock, log, and backend-path environment overrides reserved for isolated tests. Installer child processes may reuse the parent's runtime lock only through an inherited descriptor rebound to the canonical lock inode and validated for flock capability. Go self-upgrade removes the SHA-256-only escape path and always requires `gpg` from the trusted system path, the archive signature, the embedded public key, and the pinned fingerprint. Bootstrap installation and privileged self-upgrade also ignore the caller's `TMPDIR` and create private workspaces only beneath a root-owned system temporary hierarchy validated component by component without following symlinks.
- 加固发布准备与 CI 供应链：显式正式发布模式现在自动要求签名，不能因未指定签名参数而生成无签名正式产物；发布源漂移检查扩展到 `build/`、Dependabot 配置和 live-canary workflow，避免在打包期间漏过相关输入变化。生命周期 CI 使用的 Linux 容器镜像同时固定 tag 与 SHA-256 digest，减少上游可变标签导致的未审阅环境漂移。
  Hardens release preparation and the CI supply chain. Explicit official-release mode now requires signing automatically, preventing unsigned official artifacts when no signing mode was supplied. Release-source drift checks now include `build/`, Dependabot configuration, and the live-canary workflow so related input changes cannot be overlooked during packaging. Linux container images used by lifecycle CI are pinned by both tag and SHA-256 digest, reducing unreviewed environment drift from mutable upstream tags.
- 修复自动更新 watchdog 对 systemd timer 状态的判断：timer 已启用但未处于 active 状态时现在会明确告警，`ActiveState` 查询失败或返回空值时标记为无法可靠确认，而不再视为健康；active 且尚无成功运行或触发历史的 timer 仍按等待首次运行处理。
  Fixes automatic-update watchdog handling of systemd timer state. A timer that is enabled but not active now raises an explicit alert, while a failed or empty `ActiveState` query is treated as indeterminate rather than healthy. An active timer without successful-run or trigger history continues to be treated as waiting for its first run.

## 3.1.5

- 将补丁积压、整机重启和服务重启的持续时间改为 `Present`、`Absent`、`Unknown` 三态观测：只有明确不存在才清除 `first_seen`，命令失败或输出不完整会保留既有时间且本轮不触发陈旧告警。APT 待装统计现在只采信候选版本的 Debian/Ubuntu security 或 ESM 来源，不再把 `debian-security-support`、`libmodsecurity3` 等普通包名误判为安全更新；DNF 仓库故障仅匹配有上下文的 GPG/TLS 诊断，有效 DNF5 JSON 和 `gnutls`、`rustls`、`detached-signature` 等包名不再触发签名故障。
  Changes patch-backlog, host-reboot, and service-restart age tracking to explicit `Present`, `Absent`, and `Unknown` observations: only confirmed absence clears `first_seen`, while command failures or incomplete output preserve the prior timestamp and cannot trigger a stale alert in that run. APT pending counts now trust only Debian/Ubuntu security or ESM origins on the candidate version, avoiding false positives from ordinary package names such as `debian-security-support` and `libmodsecurity3`. DNF repository failures now require contextual GPG/TLS diagnostics, so valid DNF5 JSON and package names such as `gnutls`, `rustls`, or `detached-signature` cannot become signature failures.
- 将每个通知渠道的去重状态绑定到稳定接收目标，同时保持既有 11 字段告警哈希逐字节不变。Telegram 使用 bot 数字身份与 Chat ID，飞书使用 App ID 与 `open_id`，磁盘只保存域分离 SHA-256；安装器在新配置可见前为接收目标变更写入可回滚的无身份 pending 标记，失败发送不推进目标，成功发送以偏向重发的提交顺序落盘。旧状态缺少目标指纹时不会在升级后全量重发。HTTP 408 现在是临时故障：只读 Telegram/飞书请求可有界重试，会产生副作用的消息请求不立即重试；Telegram 429 遵循并封顶服务端 `parameters.retry_after`。`doctor` 也会明确显示 `OK`、`SKIP` 或 `UNKNOWN`，不再把未执行或无法确认的检查显示为成功。
  Binds each channel's deduplication state to its stable delivery target while keeping the existing 11-field alert hash byte-for-byte unchanged. Telegram uses the numeric bot identity plus Chat ID and Feishu uses App ID plus `open_id`; only a domain-separated SHA-256 is stored. Before a new configuration becomes visible, recipient changes made through the installer create a rollback-protected, identity-free pending marker; failed sends do not advance the target, and successful sends commit in a resend-biased order. Legacy state without a target fingerprint does not cause a fleet-wide resend after upgrade. HTTP 408 is now temporary: read-only Telegram/Feishu operations may retry within bounds, while side-effecting message requests do not retry immediately; Telegram 429 honors a capped server `parameters.retry_after`. `doctor` now reports `OK`, `SKIP`, or `UNKNOWN` explicitly instead of presenting skipped or indeterminate checks as successful.
- 在不改变包、导出 API 或事务顺序的前提下，按职责拆分安装器系统集成与 RootFS、交互安装 CLI、卸载/恢复四个超过 1,300 行的核心文件；新生产文件分别聚焦 systemd、APT/DNF policy、安装文件、参数/向导/预检/提示、路径校验、原子文件、xattr 和删除事务，原四个实现文件及其拆分文件均不超过 500 行。
  Splits the four core files over 1,300 lines for installer system integration and RootFS, the interactive install CLI, and uninstall/recovery into responsibility-focused same-package files without changing package boundaries, exported APIs, or transaction ordering. The resulting production files separately own systemd, APT/DNF policy, installed files, arguments/wizard/preflight/prompts, path validation, atomic files, xattrs, and removal transactions; each original implementation file and its extracted files is now below 500 lines.
...
- 加固本地发布事务：打包前后对发布源内容及精确 HEAD/tag 身份生成并复验指纹，签名前和签名后均拒绝并发源码漂移；同一输出目录的最终提交通过目录 inode 独占锁串行化，避免并发打包的备份/回滚互相删除或拼接四件产物。普通错误会尝试回滚，恢复不完整时保留证据并让后续打包失败关闭；文档明确多文件集合不承诺崩溃原子性，只在命令成功返回且无恢复目录后可供未加锁读取者使用。
  Hardens local release transactions: release-source bytes and exact HEAD/tag identity are fingerprinted and revalidated around packaging and signing, rejecting concurrent source drift both before and after signing. Final commits to the same output directory are serialized with an exclusive lock on the directory inode, preventing concurrent packagers from deleting each other's backups or mixing the four assets. Ordinary failures attempt rollback; incomplete recovery retains evidence and blocks later packaging. The multi-file set does not promise crash atomicity and is usable by unlocked readers only after successful return with no recovery directory.
- 收紧下载、归档与验签边界：完整发布资产集先暂存再提交，失败恢复旧集合；checksum、公钥和签名验证绑定同一已打开 inode。gzip 多成员、tar 尾随数据、特殊条目、路径替换和资源超限均失败关闭；所有 GPG 路径要求唯一 `GOODSIG + VALIDSIG`、固定 primary fingerprint，并拒绝多签名、过期、撤销和错误签名结果。
  Tightens download, archive, and signature boundaries: complete release sets are staged before commit and failures restore the previous set, while checksum, public-key, and signature verification stay bound to the same opened inode. Concatenated gzip members, trailing tar data, special entries, path replacement, and resource-limit violations fail closed. Every GPG path now requires one `GOODSIG + VALIDSIG` bound to the pinned primary fingerprint and rejects multiple, expired, revoked, or otherwise bad signature outcomes.
- 加固安装、升级与卸载事务：特权路径使用目录句柄和 no-follow 操作，复验 inode、所有权、权限、硬链接、xattr 及并发替换；配置、凭据、APT/DNF 基线、systemd unit/timer 状态和失败回滚保持同一事务语义。`os-release` 允许权限受保护的硬链接以兼容不可变镜像，但仍拒绝非 root、可写、非普通或超限对象。部分 Oracle Linux 8 厂商镜像的 `/etc/dnf` 默认为 `root:root 0775`，管理员须确认路径后收紧到 `0755`；测试只模拟该精确前置条件，不放宽生产信任边界。
  Hardens install, upgrade, and uninstall transactions: privileged paths use directory handles and no-follow operations with inode, ownership, permission, hard-link, xattr, and concurrent-replacement checks. Configuration, credentials, APT/DNF baselines, systemd unit/timer state, and failure rollback now share one transactional contract. Protected hard-linked `os-release` files remain compatible with immutable images while non-root-owned, writable, non-regular, or oversized objects are rejected. Some Oracle Linux 8 vendor images default `/etc/dnf` to `root:root 0775`; administrators must verify the path and tighten it to `0755`, while tests model only that exact prerequisite without weakening the production trust boundary.
- 固定所有特权外部命令的搜索路径并净化继承环境，限制 stdout/stderr 和执行时间；context 取消及 `SIGHUP/SIGINT/SIGTERM` 会清理完整进程组，隐藏输入同时恢复终端回显。容器 mock 变量只有在 `/.dockerenv`、`SUN_CONTAINER_TEST=1`、只读 `/src` 和受信命令目录全部成立时才可继承。
  Fixes privileged command lookup to a trusted path with a sanitized inherited environment, bounded stdout/stderr, and explicit timeouts. Context cancellation and `SIGHUP/SIGINT/SIGTERM` clean complete process groups, while hidden input restores terminal echo. Container fixture variables are inherited only when `/.dockerenv`, `SUN_CONTAINER_TEST=1`, a read-only `/src`, and a trusted command directory all hold.
- 收紧运行时输入与展示：本地、远端和缓存版本必须是有界规范版本；命令输出、错误、主机信息及 CLI 参数在展示前清除终端控制和双向文本控制字符。失败的 APT journal 查询不再参与签名故障分类，Ubuntu 20.04 ESM 也只有在探测完整成功时才采信。
  Tightens runtime input and display handling: local, remote, and cached versions must be bounded canonical identities, while command output, errors, host data, and CLI arguments have terminal and bidirectional controls removed before display. Failed APT journal queries no longer influence signature-failure classification, and Ubuntu 20.04 ESM status is trusted only after a complete successful probe.
- 稳定镜像发布在同一远端锁内快照旧 `sun.sh` 与 `latest.json`；公网回读失败、控制通道 EOF、超时、信号或显式 `ABORT` 会恢复二者，只有最终 `RELEASE` 才提交并清除备份。CI 直接模拟提交、中止、EOF 和首次发布四种协议状态。
  Stable mirror publication now snapshots the previous `sun.sh` and `latest.json` under the same remote lock. Public read-back failure, control-channel EOF, timeout, signal, or explicit `ABORT` restores both files; only final `RELEASE` commits and removes the backups. CI directly simulates commit, abort, EOF, and first-publication protocol states.
- 收紧通知发送的重试语义以避免重复告警：Telegram `sendMessage` 及飞书消息 POST 只在服务端明确限流时立即重试；传输中断、5xx、响应截断或非法响应改为返回临时失败，留待下一轮调度处理。只读 Telegram `getMe` 仍会对网络错误、429 和 5xx 做有界重试。
  Narrows notification-send retries to avoid duplicate alerts: Telegram `sendMessage` and Feishu message POST retry immediately only when the server explicitly rate-limits the request; transport failures, 5xx responses, truncated responses, and invalid responses now return temporary failure for a later scheduled run. Read-only Telegram `getMe` still performs bounded retries for network errors, 429, and 5xx responses.
- 修复普通用户自升级的语言降级：未显式给出 `--lang` 时不再把无权读取 root-only 配置后得到的默认中文固化并传过 sudo，而由 root 子进程重新预读安装语言；显式语言仍原样保留。文档同时明确普通用户运行 `check-upgrade` 时需要显式 `--lang` 才能匹配 root-only 配置。
  Fixes language fallback during unprivileged self-upgrade: when `--lang` is omitted, the default Chinese produced after an unreadable root-only config is no longer frozen and passed through sudo; the root child pre-reads the installed language again, while an explicit language remains unchanged. Documentation also states that an unprivileged `check-upgrade` needs an explicit `--lang` to match the root-only configuration.
- 收紧首次安装和公网 canary 的特权边界：`sun.sh` 强制 Bash `-p`，在读取脚本前禁止 `BASH_ENV` 和导出函数注入，再清除别名及非白名单环境，Python 辅助器使用隔离模式，并拒绝不唯一的有效归档签名；canary 不再卸载宿主机预先存在的 SUN，且在执行已验签发布载荷前移除 GitHub 令牌。
  Tightens privileged first-install and public-canary boundaries: `sun.sh` requires Bash `-p` so `BASH_ENV` and exported functions cannot inject code before the script is read, then removes aliases and non-allowlisted environment variables, runs Python helpers in isolated mode, and rejects a non-unique valid archive signature; the canary no longer uninstalls a pre-existing host installation and drops its GitHub token before executing verified release payloads.
- 加固无部署权限的 Release 验证：公开资产经 HTTPS 流式双重限流下载，不再由无大小上限的 `gh release download` 先落盘；验签后的多架构版本探针在空环境、固定 PATH、超时及 4 KiB 输出上限内执行，并要求输出逐字节等于唯一预期行。持有 `release-mirror` Environment 的全新 runner 会从头复验资产但不执行发布载荷。
  Hardens unprivileged Release verification: public assets use streaming HTTPS downloads with two independent byte limits instead of being written first by an unbounded `gh release download`; authenticated multi-architecture version probes run with an empty environment, fixed PATH, timeout, and 4 KiB output cap, and their output must byte-match exactly one expected line. A fresh runner holding the `release-mirror` Environment revalidates the assets from scratch without executing release payloads.
...
- 公网 canary 的只读 `apt-get check` 现在会最多等待 300 秒 dpkg 锁，并受 330 秒外层超时限制，避免将托管 runner 上刚启动的 `unattended-upgr` 竞争误报为包状态损坏；dpkg audit 仍保持原有 60 秒上限，实际依赖错误、锁超时和 audit 异常仍失败关闭。
  The public canary's read-only `apt-get check` now waits up to 300 seconds for the dpkg lock under a 330-second outer timeout, avoiding a false package-state regression when `unattended-upgr` has just started on a hosted runner. The dpkg audit retains its 60-second bound, and real dependency failures, lock timeouts, and audit findings still fail closed.
