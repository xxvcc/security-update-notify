...
- 加固安装、升级与卸载事务：特权路径使用目录句柄和 no-follow 操作，复验 inode、所有权、权限、硬链接、xattr 及并发替换；配置、凭据、APT/DNF 基线、systemd unit/timer 状态和失败回滚保持同一事务语义。`os-release` 允许权限受保护的硬链接以兼容不可变镜像，但仍拒绝非 root、可写、非普通或超限对象。
  Hardens install, upgrade, and uninstall transactions: privileged paths use directory handles and no-follow operations with inode, ownership, permission, hard-link, xattr, and concurrent-replacement checks. Configuration, credentials, APT/DNF baselines, systemd unit/timer state, and failure rollback now share one transactional contract. Protected hard-linked `os-release` files remain compatible with immutable images while non-root-owned, writable, non-regular, or oversized objects are rejected.
