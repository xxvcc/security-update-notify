...
Best-effort means the SUN code path is compatible; administrators must still confirm that subscriptions and security sources are valid. Ubuntu 20.04 checks local `esm-infra` state automatically. Amazon Linux 2023 still requires the administrator to track and advance release snapshots. Unlisted `ID_LIKE` derivatives are never promoted to official support automatically; see [Operations and recovery](docs/operations.en.md) for detection and rollback rules.
